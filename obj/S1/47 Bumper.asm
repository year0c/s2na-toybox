...
Bump_Hit:	; Routine 2
		move.b	obColProp(a0),d0	; has Sonic touched the bumper?
...
.hit:
		...
		muls.w	#-$700,d1
		asr.l	#8,d1
		move.w	d1,obVelX(a1)	; bounce player away
		muls.w	#-$700,d0
		asr.l	#8,d0
		move.w	d0,obVelY(a1)	; bounce player away
		bset	#1,obStatus(a1)
		bclr	#4,obStatus(a1)
		bclr	#5,obStatus(a1)
		clr.b	jumping(a1)
		...
		bhs.s	.return	; if yes, player gets no points
		addq.b	#1,2(a2,d0.w)
...
.addscore:
		...
		bsr.w	FindFreeObj
		bne.s	.return
		_move.b	#id_Obj29,obID(a1) ; load points object
